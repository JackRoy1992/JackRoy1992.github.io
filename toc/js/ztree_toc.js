...
(function ($) {
  ...
  function render_with_ztree(opts) {
    var t = $(opts._zTree);
    t = $.fn.zTree
      .init(t, opts.ztreeSetting, opts._header_nodes)
      .expandAll(opts.is_expand_all);
  ...
  function _rename_header_content(opts, header_obj, level) {
    ...
    if (opts.is_auto_number == true) {
      ...
      if ($(header_obj).text().indexOf(opts._headers.join(".")) != -1) {
        ...
        $(header_obj).text(
          opts._headers.join(".") + ". " + $(header_obj).text()
        );
      }
    }
  }
  ...
  function _add_header_node(opts, header_obj) {
    ...
    opts._header_offsets.push(
      $(header_obj).offset().top - opts.highlight_offset
    );
      ...
      timeout = setTimeout(function () {
        ...
        for (var i = 0, c = opts._header_offsets.length; i < c; i++) {
          ...
          if (opts._header_offsets[i] >= top + 5) {
            console.log(
              "opts._header_offsets[" + i + "] = " + opts._header_offsets[i]
            );
